...
export function Fretboard({ gegeben = [], gewaehlt, loesung, onPick }: FretboardProps) {
  const breite = RAND_LINKS + BUENDE * BREITE + 14
  const hoehe = RAND_OBEN + 6 * HOEHE + 4
  ...
  return (
    <div className="-mx-1 overflow-x-auto px-1 py-1">
      ...
      >
        ...
        {EINLAGEN.map((bund) => (
          <circle key={bund} cx={x(bund)} cy={y(3.5)} r={7} fill="var(--panel2)" />
        ))}
        <circle cx={x(12)} cy={y(2.2)} r={6} fill="var(--panel2)" />
        <circle cx={x(12)} cy={y(4.8)} r={6} fill="var(--panel2)" />
